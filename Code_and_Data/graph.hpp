...
public:
  ...
  void write_result(char const *filename,  vector<pair<int,double>>const& sssp_time, vector<double> & djk_times)
  {
    printf("Info: Writing result\n");
    FILE *fp = fopen(filename, "w");
    for(size_t i = 0; i < sssp_time.size(); ++i)
    {
      int _s = sssp_time[i].first;
      double _time = sssp_time[i].second;
      double djk_t = djk_times[i];
      fprintf(fp, "%d %f %f\n", _s, _time, djk_t);
    }
    fclose(fp);
  }
  void write_result(char const *filename,  vector<pair<int,double>>const& sssp_time)
  {
    printf("Info: Writing result\n");
    FILE *fp = fopen(filename, "w");
    for(auto & [_s, _time] : sssp_time)
    {
      fprintf(fp, "%d %f\n", _s, _time);
    }
    fclose(fp);
  ...
  void check_symmetrized()
  {
    ...
    {
      auto type = [](bool s)
      { return s ? "symmetrized" : "unsymmetrized"; };
